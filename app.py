...
history_table = dash_table.DataTable(
    id="history",
    columns=(
        [{"id": "cash_alloc", "name": "Cash Allocation", "type": "text"}]
        + [{"id": "stock_alloc", "name": "Stock Allocation", "type": "text"}]
        + [{"id": "start_amount", "name": "Start Amount", "type": "text"}]
        + [{"id": "start_year", "name": "Start Year", "type": "text"}]
        + [{"id": "num_years", "name": "Number of Years", "type": "text"}]
    ),
    data=None,
    sort_action="native",
    page_size=15,
...
def make_bar(slider_input, title):
    investment_types = ["Cash", "Bonds", "Stocks"]
    colors = ["#3cb521", "#fd7e14", "#446e9b"]
    # print(colors)
    fig = go.Figure(
        data=(
            go.Bar(
                x=slider_input,
                text=[str(v) for v in investment_types],
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Investment Type"),
        title_x=0.5,
...
def backtest(stocks, cash, start_bal, nper, start_yr):
    ...
    for yr in dff.Year + 1:
        if yr <= end_yr:
            ...
            dff.loc[yr, "Total"] = dff.loc[yr, ["Cash", "Bonds", "Stocks"]].sum().astype(int)  # stops future warning
...
app.layout = dbc.Container(
    [
        dcc.Store(id="history_data", data=starting_data.to_dict('records')),
        dcc.Store(id="history_freeze", data=False),
        dbc.Row(
            dbc.Col(
                [
                    html.Div(
                        [
                            ...
                            html.H4(
                                "Joshua Nielsen - CS 150 Community Action Computing",
                                className="text-white p-2"
                            ),
                        ],
...
def update_bar(stocks, cash):
    ...
    figure = make_bar(slider_input, investment_style + " Asset Allocation")
    return figure, bonds
...
@app.callback(
    ...
    Output("history_freeze", "data"),
    State("history_data", "data"),
    State("history_freeze", "data"),
    Input("cash", "value"),
    Input("stock_bond", "value"),
    Input("starting_amount", "value"),
    Input("planning_time", "value"),
...
def log_history(current_data, history_freeze, cash_alloc, stock_alloc, start_amount, num_years, start_year):
    if history_freeze:
        return current_data, False
    elif not all([cash_alloc, stock_alloc, start_amount, num_years, start_year]):
        return current_data, False
    new_row = {
...
@app.callback(
    Output("history_freeze", "data", allow_duplicate=True),
    ...
    Output("history_data", "data", allow_duplicate=True),
    Input("back-button", "n_clicks"),
    State("history_data", "data"),
    prevent_initial_call=True
)
def step_backwards(n_clicks, data):
    if not data or len(data) < 2:
        return dash.no_update

    new_data = pd.DataFrame(data)
    last_row = new_data.iloc[-2].tolist()
    new_data = new_data.drop(new_data.index[-1])
    new_data = new_data.to_dict("records")

    return True, *last_row, new_data


# Please gnore this part, history_freeze took hours to implement, so I'm leaving this here as evidence.
